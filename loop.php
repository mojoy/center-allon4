<div class="posts__item"  itemscope="" itemtype="http://schema.org/Article">
    <div class="posts__item-img"  itemprop="image" itemscope="" itemtype="https://schema.org/ImageObject">
        ...
        $w = 480; $h = 300;
    	if ( kama_thumb_src() ) {
    	    echo '<img src="'.kama_thumb_src('w='.$w.'&h='.$h).'" width="'.$w.'" height="'.$h.'" alt="'.get_the_title().'" itemprop="image contentUrl Url" loading="lazy" decoding="async" />';
    	} else {
    	    echo '<img src="'.get_stylesheet_directory_uri().'/images/no-photo.jpg" width="'.$w.'" height="'.$h.'" alt="Изображение для публикации не задано" loading="lazy" decoding="async">';
    	} ?>
